<template>
	<div>
		...
		<div class="button-box">
			...
			<el-button @click='daoru()' type='' icon="el-icon-upload2" class='btn-daoru' style='margin-bottom: 15px;'>批量导入</el-button>
			<el-button @click='updata()' type='primary' icon="el-icon-download" class='btn-updata' style='margin-bottom: 15px;'>模板下载</el-button>
		</div>
...
<script>
	...
	export default {
			...
			return {
				...
				columns: [{
					...
					},
					{
						...
						type: 'img',
						renderContent: (h, { row }) => {
							if(row.headPic&&row.headPic.indexOf('?uploadId')){
		...
		methods: {
			...
			updata(){
				account.StudentUpdata().then(res => {
					console.log(res)
//					if(res.data.code === 0) {
//						
//					}
