<template>
	<div>
		...
		<DataTable class='my-table' :loading="loading" :columns="columns" :data="data" :total="total" :page-size="search.size" @page-change="pageChange"  @selection-change="selectionChange"></DataTable>
...
//					},
					...
					},
					{
						...
						renderContent: (h, { row }) => {
							return [
								<el-button type='primary' on-click={() => this.$router.push(`readBook/see/${row.id}`)}   size='mini'>查看</el-button>,
								<el-button type='' on-click={() => this.$router.push(`readBook/edit/${row.id}`)} style='border:1px solid #409EFF; color:#409EFF' size='mini' >更新</el-button>,
		...
		methods: {
			...
			selType(){
            	booksSet.readSel(this.types).then(res => {
...
//					typeId:'',
           		...
           		booksSet.bookSel(books).then(res => {
...
//		        	console.log(this.deleteArr)
		        	...
		        	this.global.deleteConfirm.call(this,function(){
		        		booksSet.Delete2S(ids).then(res => {
					          if(res.data.code === 0) {
