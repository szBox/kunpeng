<template>
  <div class="add-edit-form-box">
    <el-form>
      <el-form-item>
        <template>教师名字：<span>{{g.name}}</span></template>
      </el-form-item>
      <el-form-item>
        <template>电话号码：<span>{{g.phone}}</span></template>
      </el-form-item>
      <el-form-item>
        <template>部门名称：<span>{{g.depName}}</span></template>
      </el-form-item>
      <el-form-item>
        <template>性别：<span v-if="g.sex==1">男</span><span v-if="g.sex==2">女</span></template>
      </el-form-item>
      <!--<el-form-item>-->
        <!--<template>所带班级：<span>{{g.className}}</span></template>-->
      <!--</el-form-item>-->
      ...
      <el-form-item>
        <template>特长：<span>{{g.skill}}</span></template>
      </el-form-item>
      <el-form-item>
        <template>点赞数：<span>{{g.praiseCount}}</span></template>
      </el-form-item>
      <el-form-item>
...
<script>
  ...
  export default {
    ...
    created(){
      let vm = this;
      teacher.queryById.r(vm.id).then(function (res) {
        let data = res.data;
        if (data.code == 0) {
          vm.g = data.data;
        }
