					...
					
					{
						...
						renderContent: (h, {
							...
							return [
								...
								<el-button type='' on-click={() => this.$router.push(`teacher/edit/${row.id}`)} style='border:1px solid #409EFF; color:#409EFF'  size='mini' >更新</el-button>,
		...
		methods: {
			...
			tableList() {
				identity.tableIdentity(this.search).then(res => {
					console.log('教师身份表格', res) //接受到的参数
					if(res.data.code === 0) {
						this.loading = false;
						for(var i=0; i<res.data.data.records.length; i++ ){
							this.$set(res.data.data.records[i], 'schoolName', this.global.session.get('user').shoolName);
						}
