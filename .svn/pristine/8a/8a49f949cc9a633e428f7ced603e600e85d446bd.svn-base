...
//								<el-button type='primary' on-click={() => this.$router.push({ name: 'booksTypeSee', params: { init: row}})}  size='mini'>查看</el-button>,
								<el-button type='primary' on-click={() => this.$router.push({ name: 'booksTypeEdit', params: { init: row }})} size='mini' >更新</el-button>,
		...
		methods: {
		        ...
		        if(this.deleteArr.length === 0) {
		          ...
		          return;
		        	...
		        	this.global.deleteConfirm.call(this,function(){
		        		booksType.DeleteS(ids).then(res => {
					          if(res.data.code === 0) {
			...
			typeList(){
				booksType.booksTyesList(this.selectBox).then(res => {
					if(res.data.code === 0) {
